...
import { getFriendByUserId } from "@/lib/helpers/get-friend-by-user-id"
  ...
  return (
    <div className="w-full flex h-screen">
      <div className="md:hidden">
        <MobileChatLayout sidebarOptions={sidebarOptions} unseenRequestCount={unseenRequestCount} friends={friends} session={session}/>
      </div>
        <div className="hidden md:flex h-full w-full max-w-xs grow flex-col gap-y-5 overflow-y-auto border-r border-x-gray-200 bg-white px-6">
            <Link href='/dashboard' className="flex h-16 shrink-0 items-center">
              <Icons.Logo className='h-8 text-indigo-600 w-auto'/>
            </Link>
                ...
                <li>
                 ...
                 <ul role="list" className="-mx-2 mt-2 space-y-1">
                  {sidebarOptions.map((option)=>{
                    ...
                    return(
                      <li key={option.id}>
                        <Link href={option.href} className="text-gray-700 hover:text-indigo-600 hover:bg-slate-50 group flex gap-3 rounded-md p-2 text-sm leading-6 font-semibold">
                ...
                <li className="-mx-6 mt-auto flex">
                  <div className="flex flex-1 items-center gap-x-4 px-6 py-3 text-sm font-semibold leading-6 text-gray-900">
                    <div className="relative h-8 w-8 bg-gray-50">
                      <Image fill referrerPolicy="no-referrer" className="rounded-full" src={session.user.image || ''} alt="user-image"/>
                    </div>
